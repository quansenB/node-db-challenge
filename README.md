...
1. Explain the difference between `Relational Databases` and `SQL`.
  A relational Database is a structures Database, SQL is the language in whcih we can "talk" to the database.
1. Why do tables need a `primary key`?
  A primary key serves as the identifier, we can sue it for example in another table to have a link for yoining the tables when we need to.
1. What is the name given to a table column that references the primary key on another table.
  Foreign key.
1. What do we need in order to have a _many to many_ relationship between two tables.
  We need a intermediary table which specifies which entries(via their primary keys) do have relationships.
